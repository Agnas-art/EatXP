...
serve(async (req) => {
  ...
  try {
    const { message, conversationHistory, conversationSummary, conversationMetadata, recentContext, isInternalSummary } = await req.json();
    
    console.log("Received message:", message);
    console.log("Conversation history length:", conversationHistory?.length || 0);
    console.log("Has conversation summary:", !!conversationSummary);
    console.log("Has conversation metadata:", !!conversationMetadata);
    console.log("Has recent context:", !!recentContext);
    if (conversationMetadata) {
      console.log("Message count:", conversationMetadata.messageCount);
      console.log("User name:", conversationMetadata.userName || 'Not provided');
    }
    console.log("Has conversation metadata:", !!conversationMetadata);
    console.log("Has recent context:", !!recentContext);
    if (conversationMetadata) {
...
Be conversational and maintain context throughout our chat.`;
    ...
    if (isInternalSummary) {
      ...
      if (recentContext) {
        systemPrompt += `\n\nRECENT CONVERSATION FLOW: ${recentContext}\n\nBuild upon this recent discussion naturally.`;
      }
    } else if (recentContext) {
      systemPrompt += `\n\nRECENT CONVERSATION: ${recentContext}\n\nContinue this conversation naturally and reference what we've been discussing.`;
    }
